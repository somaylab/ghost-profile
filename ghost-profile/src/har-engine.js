/**
 * Ghost Profile — har-engine.js
 * ═══════════════════════════════════════════════════════════════
 * Global Multi-Tab HAR & Interaction Flow Recorder Engine
 * 
 * Capabilities:
...
(function (global) {
  ...
  function initWebRequestListeners() {
    ...
    }, { urls: ['<all_urls>'] }, ['requestHeaders', 'extraHeaders'].filter(Boolean));
    ...
    }, { urls: ['<all_urls>'] }, ['responseHeaders', 'extraHeaders'].filter(Boolean));
  ...
  function startRecording() {
    ...
    broadcastLiveStats();
    console.log('[Ghost HAR Engine] Global recording started.');
    return true;
  }
  ...
  function stopRecording() {
    ...
    broadcastLiveStats();
    console.log('[Ghost HAR Engine] Global recording stopped.');
    return true;
  }
  ...
  function clearRecording() {
    ...
    broadcastLiveStats();
    console.log('[Ghost HAR Engine] Recording buffer cleared.');
    return true;
  }
  ...
  function buildHarLog(customCreator = {}) {
    ...
    const harObject = {
      log: {
        version: '1.2',
        creator: {
          name: customCreator.name || 'Ghost Profile HAR & Flow Engine',
          version: customCreator.version || '3.2.0',
          comment: 'Global Multi-Tab & Interaction Flow Archive'
        },
        browser: {
          name: navigator.userAgent.includes('Firefox') ? 'Firefox' : 'Chrome',
          version: (navigator.userAgent.match(/(?:Chrome|Firefox)\/(\d+(\.\d+)*)/) || [])[1] || '1.0'
        },
        pages: pages,
