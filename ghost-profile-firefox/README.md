# 🦊 Ghost Profile — Mozilla Firefox Edition (v4.4.0)
...
[![Anti-Bot Audit](https://img.shields.io/badge/Anti--Bot%20Audit-100%25%20Passed-brightgreen.svg)](https://github.com/somaylab/ghost-profile/releases/tag/v4.4.0)
...
## ⚡ v4.4.0 Zero-Flag Hardening Upgrades (Gecko Hardened)

* 📐 **Realistic Viewport & Window Geometry**: Full parity between `window.innerWidth`/`innerHeight`, `document.documentElement.clientWidth`/`clientHeight`, and `VisualViewport` (screen minus browser chrome), eliminating viewport mismatch flags.
* 🛡️ **Adobe Commerce GraphQL Promo Interceptor**: Automatically strips stale `promotionCodes` from checkout mutation payloads to prevent geo/currency pricing mismatch errors (`INVALID_CONCESSION`).
* 🎲 **Mulberry32 Deterministic Storage**: `StorageManager.estimate()` now uses deterministic Mulberry32 PRNG keyed to canvas noise seed, completely eliminating non-deterministic entropy leaks.
